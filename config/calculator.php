...
use iEXPackages\Calculator\Strategies\DerivedBaselineStrategy;
...
use iEXPackages\Calculator\Strategies\ZelleBaselineStrategy;

return [

    'priority' => array_values(array_filter(array_map(
        static fn ($v) => trim((string) $v),
        explode(',', env('CALCULATOR_SOURCE_PRIORITY', 'zelle,derived,bestchange,crypto,formula,manual,file,competitor'))
    ))),


    'sources' => [
        'zelle' => [
            'strategy' => ZelleBaselineStrategy::class,
            'check' => [SourceChecks::class, 'zelle'],
            'name' => [SourceChecks::class, 'zelleName'],
        ],
        'derived' => [
            'strategy' => DerivedBaselineStrategy::class,
            'check' => [SourceChecks::class, 'derived'],
            'name' => [SourceChecks::class, 'derivedName'],
        ],
        'bestchange' => [
