...
use App\Http\Resources\Admin\Orders\OrderCommentsUserResources;
...
class OrderCommentsUserController extends Controller
{
    /**
     * Получаем список всех комментариев
     *
     * @param int $id
     * @return OrderCommentsUserResources
     */
    public function index(int $id)
    {
        $order = TaskCommentUser::where('id_task', $id)->get();
        return new OrderCommentsUserResources($order ?? []);
    }
    ...
    {
        $item = TaskCommentUser::findOrFail($id);
        $item->delete();
