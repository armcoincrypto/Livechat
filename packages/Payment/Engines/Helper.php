...
class Helper
{
     ...
     */
    public static function getGatewayShortName($className): string
    {
        if (strpos($className, '\\') === 0) {
            $className = substr($className, 1);
        }

        if (strpos($className, 'iEXPackages\\Payment\\Gateways\\') === 0) {
    ...
    public static function getGatewayClassName(string $shortName): string
    {
        ...
        return '\\iEXPackages\\Payment\\Gateways\\'.$shortName.'Gateway';
    }
}
