...
final class BestchangeUpdateRatesConsole extends Command
{
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $changed = (int) $ratesConnection->getCountUpdateData();
            $freshnessSeconds = max(60, (int) iEXSetting('cron_interval_minutes_update_rates', 1) * 60);
            $lastFullAt = (int) Cache::get('iex:compiler:bestchange:last_full_at', 0);
            $withinFreshness = $lastFullAt > 0 && (time() - $lastFullAt) < $freshnessSeconds;

            // When the BestChange payload produced no row changes and a full
            // recalculation already completed inside the freshness window, skip
            // direction recalculation. Rate formulas and policy are unchanged.
            if ($changed === 0 && $withinFreshness) {
                $elapsed = round(microtime(true) - $start, 4);
                $this->info("Нет изменений курсов BestChange — пересчёт направлений пропущен ({$elapsed} сек).");
                return self::SUCCESS;
            }

            // 2) пересчитываем direction_exchange
            $ratesConnection->withUpdateDirections();
            Cache::put('iex:compiler:bestchange:last_full_at', time(), 86400);
