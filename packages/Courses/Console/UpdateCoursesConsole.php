...
final class UpdateCoursesConsole extends Command
{
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            // ionCube Rates may copy the USDTTRC20→dest benchmark peer's
            // parser_source_name onto ZELLEUSD outgoing rows. Re-assert exclusive
            // ZELLE ownership immediately so labels cannot oscillate to DERIVED/manual.
            try {
                $zelle = ZelleUsdUsdtBenchmarkAuthority::make()->refreshAll(dryRun: false);
                $this->comment(sprintf(
                    'ZELLE reassert after courses: scanned=%d ok=%d failed=%d',
                    (int) ($zelle['scanned'] ?? 0),
                    (int) ($zelle['ok'] ?? 0),
                    (int) ($zelle['failed'] ?? 0)
                ));
            } catch (Throwable $zelleEx) {
                Log::error('compiler:courses zelle reassert failed', [
                    'message' => $zelleEx->getMessage(),
                ]);
                $this->warn('ZELLE reassert failed: ' . $zelleEx->getMessage());
            }
