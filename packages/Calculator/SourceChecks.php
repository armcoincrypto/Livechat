...
final class SourceChecks
{
    ...
    /**
     * Exclusive ZELLEUSD→* ownership — must win before derived/manual inside courses.
     */
    public static function zelle(object $dir): bool
    {
        if (!self::isZelleOutgoing($dir)) {
            return false;
        }

        $course = trim((string) ($dir->course_value ?? ''));
        $mirror = trim((string) ($dir->manual_rate_value ?? ''));

        return self::isPositiveNumeric($course) || self::isPositiveNumeric($mirror);
    }

    public static function zelleName(object $dir): string
    {
        return ZelleUsdUsdtBenchmarkAuthority::PARSER_SOURCE_NAME;
    }

    /**
     * DERIVED_MARKET_BASELINE ownership — wins over BestChange/manual labelling
     * inside the courses compiler so automatic BASE authority stays durable.
     */
    ...
    public static function derivedName(object $dir): string
    {
        return 'DERIVED_MARKET_BASELINE';
    }
    ...
    {
        // ZELLE / derived-owned directions must never fall through to «Ручной курс».
        if (self::isZelleOutgoing($dir) || self::isDerivedOwned($dir)) {
            return false;
        }
        ...
        return self::isPositiveNumeric($raw);
    }

    private static function isZelleOutgoing(object $dir): bool
    {
        try {
            return ZelleOutgoingRateWriteGuard::isZelleOutgoingDirection($dir);
        } catch (Throwable) {
            return false;
        }
    }

    private static function isDerivedOwned(object $dir): bool
    {
        $id = (int) ($dir->id ?? 0);
        if ($id <= 0) {
            return false;
    ...
    private static function isPositiveNumeric(string $raw): bool
    {
        if (
            $raw === '' ||
