...
class Calculator implements Arrayable
{
    ...
    public function calculate(): static
    {
        ...
        // ZELLEUSD outgoing: base = USDTTRC20→dest benchmark only.
        // Do not apply legacy profit/add_course/group fees — floating_fee/fix_fee
        // are applied once later via type_rate (orders) or CanonicalDirectionRateCalculator (website).
        if ($this->applyZelleUsdCanonicalBase()) {
            return $this;
        }
    ...
    /**
     * ZELLEUSD→dest: set rateValue to the USDTTRC20 benchmark base (no legacy profit/add_course).
     * Fees remain in type_rate options for calculateWithOptions / UI.
     */
    protected function applyZelleUsdCanonicalBase(): bool
    {
        try {
            $resolver = \App\Services\Rates\ZelleUsdBenchmarkResolver::make();
            if (!$resolver->isZelleOutgoing($this->directionExchange)) {
                return false;
            }

            $bench = $resolver->resolve($this->directionExchange);
            if (!$bench->eligible || $bench->benchmarkRate === null
                || !is_numeric($bench->benchmarkRate)
                || bccomp((string) $bench->benchmarkRate, '0', 18) <= 0
            ) {
                $this->rateValue = '0';
            ...
            $this->rateValue = (string) $bench->benchmarkRate;
            ...
            \Illuminate\Support\Facades\Log::error('zelle_calculator_canonical_base_failed', [
                'direction_id' => $this->directionExchange->id ?? null,
                'message' => $e->getMessage(),
            ]);
            if ((int) ($this->directionExchange->id_currency1 ?? 0)
                === \App\Services\Rates\ZelleUsdBenchmarkResolver::ZELLE_CURRENCY_ID
            ) {
                $this->rateValue = '0';
                $this->options = [];
...
//                ]);
    ...
    protected function getTypeRates(): array
    {
        ...
        // Release A: fix_fee / floating_fee are always percentage units.
        // Normalize bare "-3" → "-3%" so CalculatorMathService never applies absolute destination units.
        $canonical = \App\Services\Rates\CanonicalDirectionRateCalculator::make()
            ->percentExpressionsForCalculator($this->directionExchange);

        return [
            'fixed' => $canonical['fixed'],
            'floating' => $canonical['floating'],
        ];
    }
    ...
    protected function resolveFirstCourseSourceWithPositiveRate(DirectionExchange $directionExchange): array
    {
        ...
        $priorityList = $this->priorityList($sourcesConfig);
        $skippedSourceKeys = [];
    ...
    public function calculateWithOptions(array $options = []): static
    {
        ...
        // ZELLEUSD: benchmark base + fees (no city/profit/legacy stack).
        // FLOATING → floating_fee only; FIXED → floating_fee then fix_fee (owner policy).
        try {
            $zelleResolver = \App\Services\Rates\ZelleUsdBenchmarkResolver::make();
            if ($zelleResolver->isZelleOutgoing($this->directionExchange)) {
                $this->calculate();
                ...
                if (isset($options['type_rate'])) {
                    ...
                    if ($typeRateKey === 'floating') {
                        if (isset($this->options['type_rate']['floating'])) {
                            $typeRateValue = $this->options['type_rate']['floating'];
                            if ($this->isValidFlexibleMathExpression($typeRateValue)) {
                                $mathValue->calculate($typeRateValue);
                            }
                        }
                    } else {
                        // FIXED: apply shared ZELLE commercial floating_fee, then additional fix_fee.
                        foreach (['floating', 'fixed'] as $feeKey) {
                            if (!isset($this->options['type_rate'][$feeKey])) {
                                continue;
                            }
                            $typeRateValue = $this->options['type_rate'][$feeKey];
                            if ($this->isValidFlexibleMathExpression($typeRateValue)) {
                                $mathValue->calculate($typeRateValue);
                            }
                        }
            ...
            \Illuminate\Support\Facades\Log::error('zelle_calculator_with_options_failed', [
                'direction_id' => $this->directionExchange->id ?? null,
                'message' => $e->getMessage(),
            ]);
            if ((int) ($this->directionExchange->id_currency1 ?? 0)
