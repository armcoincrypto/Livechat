...
<?php
/**
 * SEO-9D.1 — Optimize authority/news listing articles (content + snapshots).
...
$configPath = __DIR__ . '/../storage/app/seo/news-seo9d1-config.json';
...
foreach ($config['articles'] as $article) {
    ...
    if ($outro !== '' && !str_contains($text, 'news-seo9d1-cta')) {
        $text .= $outro;
    }
    ...
    $snapshot = '<div class="news-seo9d1-ssr"><h1 class="text-2xl font-semibold mb-4">' . htmlspecialchars($h1, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</h1>' . $text . '</div>';
