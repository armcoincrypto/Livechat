#!/usr/bin/env bash
#
# P-UI-MASTER-23 — Sync active production CSS to Angular dist mirrors
#
set -euo pipefail
...
CACHE_VERSION="${PREMIUM_UI_CACHE_VERSION:-20260627c4}"

[[ -f "$CANONICAL" ]] || { echo "[premium-ui-sync-active] ERROR: missing $CANONICAL" >&2; exit 1; }
...
if ! grep -q "$EXPECTED" /etc/nginx/snippets/exswaping-premium-ui-overrides.conf 2>/dev/null; then
  echo "[premium-ui-sync-active] WARN: nginx snippet does not reference ?v=$CACHE_VERSION — run premium-ui-restore-active.sh"
fi
...
for dest in "${MIRRORS[@]}"; do
  ...
  echo "[premium-ui-sync-active] synced -> $dest"
done

echo "[premium-ui-sync-active] DONE ($(wc -c < "$CANONICAL") bytes, ?v=$CACHE_VERSION)"
