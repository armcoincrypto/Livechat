#!/usr/bin/env bash
# SEO-AUTHORITY-3 — Monitoring mega-hub validation
set -euo pipefail
...
GUIDE_URL="${BASE_URL}/ru/guides/monitoring-kriptovalyutnyh-obmennikov"
SITEMAP_PATH="${SITEMAP_PATH:-public/static/seo/sitemap.xml}"
MIN_WORDS=2500
MAX_WORDS=4000

FAIL=0
log(){ echo "[authority3-monitoring] $*" >&2; }
pass(){ log "PASS $*"; }
...
grep -qi 'rel="canonical" href="https://exswaping.com/ru/guides/monitoring-kriptovalyutnyh-obmennikov"' <<<"$body" && pass "self canonical" || fail "canonical"
[[ "$(grep -oi '<h1' <<<"$body" | wc -l | tr -d ' ')" == "1" ]] && pass "one H1" || fail "H1 count"
grep -qi 'Мониторинги криптовалютных обменников' <<<"$body" && pass "H1 topic" || fail "H1 topic"

title="$(grep -oiE '<title>[^<]+' <<<"$body" | head -1 | sed 's/<title>//i')"
[[ "$title" == *"Мониторинг"* && "$title" == *"Exswaping"* ]] && pass "title OK" || fail "title: $title"
...
grep -qiE 'лучший обменник|лучшая платформа|лучший мониторинг' <<<"$body" && fail "superlative ranking language" || pass "no ranking superlatives"
...
for slug in \
  ...
  grep -q "/ru/blog/${slug}" <<<"$body" || fail "missing spoke $slug"
done
pass "spoke article links"

for section in \
  "Что такое мониторинги" \
  "Как мониторинги проверяют" \
  "Зачем мониторинги" \
  "BestChange" \
  "Crypto.ru" \
  "Exnode" \
  ...
  "оценивать рейтинги" \
  "Риски и ограничения" \
  "Чеклист безопасности"; do
  grep -qi "$section" <<<"$body" && pass "section: $section" || fail "missing section: $section"
done

[[ "$(fetch_status "${BASE_URL}/ru/guides/bezopasnyj-kriptoobmen")" == "200" ]] && pass "trust hub 200" || fail "trust hub not 200"
...
grep -q '/ru/guides/monitoring-kriptovalyutnyh-obmennikov' "$SITEMAP_PATH" && fail "guide in sitemap" || pass "guide deferred"

if [[ "$FAIL" -gt 0 ]]; then echo "SEO_AUTHORITY_3_STATUS=FAIL failures=$FAIL" >&2; exit 2; fi
echo "SEO_AUTHORITY_3_STATUS=PASS" >&2
log "RESULT PASS"
exit 0
